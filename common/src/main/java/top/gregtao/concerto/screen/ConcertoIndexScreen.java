...
public class ConcertoIndexScreen extends ConcertoScreen {
    ...
    protected void init() {
        ...
        this.addButton(Component.translatable("concerto.screen.main_list"), leftX, myPlaybackY, buttonWidth,
                button -> Minecraft.getInstance().gui.setScreen(new MainPlaylistScreen(this)));
        ...
        this.addButton(Component.translatable("concerto.screen.index.qq"), rightX, this.onlineMusicY, buttonWidth,
                button -> Minecraft.getInstance().gui.setScreen(new QQMusicUserScreen(this)));
        ...
        this.addButton(Component.translatable("concerto.screen.preset_radios"), leftX, this.serverActionsY, buttonWidth,
                button -> Minecraft.getInstance().gui.setScreen(new PresetRadiosScreen(this))).active = serverAvailable;
        this.addButton(Component.translatable("concerto.screen.audition"), rightX, this.serverActionsY, buttonWidth,
                button -> Minecraft.getInstance().gui.setScreen(new MusicAuditionScreen(this))).active =
                serverAvailable && player.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER);
        ...
        this.addButton(Component.translatable("concerto.screen.rooms"), rightX,
                this.serverActionsY + BUTTON_HEIGHT + ROW_GAP, buttonWidth,
                button -> Minecraft.getInstance().gui.setScreen(new MusicRoomsScreen(this))).active = serverAvailable;
    ...
    public void extractRenderState(GuiGraphicsExtractor matrices, int mouseX, int mouseY, float delta) {
        ...
        matrices.text(this.font, Component.translatable("concerto.screen.section.my_playback"),
                this.contentX, 22, SECTION_LABEL_COLOR, false);
        matrices.text(this.font, Component.translatable("concerto.screen.section.online_music"),
                this.contentX, this.onlineMusicY - 11, SECTION_LABEL_COLOR, false);
        matrices.text(this.font, Component.translatable("concerto.screen.section.server_management"),
        ...
        if (MusicRoom.clientGetState() == MusicRoom.ClientState.MUSIC_AGENT) {
            matrices.centeredText(this.font, Component.translatable("concerto.screen.in_music_agent"), this.width / 2,
