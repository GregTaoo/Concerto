...
public class MainPlaylistScreen extends ApplyDraggedFileScreen {
    ...
    private void openTitleEditor(MainPlaylistWidget.Entry entry) {
        Minecraft.getInstance().gui.setScreen(new EditMusicTitleScreen(entry.music().getMeta().title(), title -> {
    ...
    public void extractRenderState(GuiGraphicsExtractor matrices, int mouseX, int mouseY, float delta) {
        ...
        this.volumeControl.extractRenderState(matrices, mouseX, mouseY, delta);
    }
