...
public class MusicInfoScreen extends ConcertoScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        this.headPicture.extractRenderState(graphics, mouseX, mouseY, delta);
    ...
    private void renderInfoPanel(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
        ...
        if (meta == null) {
            graphics.text(this.font, Component.translatable("concerto.screen.loading"), textX, textY, 0xffaaaaaa, false);
            return;
        }

        graphics.text(this.font, this.font.plainSubstrByWidth(meta.getSource(), textWidth), textX, textY, 0xff55ffff, false);
        textY += 14;
        ...
        for (FormattedCharSequence line : this.font.split(Component.literal(meta.title()), textWidth)) {
            ...
            graphics.text(this.font, line, textX, textY, 0xffffffff);
            textY += 10;
        }
        if (!meta.author().isEmpty()) {
            textY = Math.min(textY + 2, textBottom - 10);
            graphics.text(this.font, this.ellipsize(meta.author(), textWidth), textX, textY, 0xffaaaaaa, false);
        }
    }
    @Override
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        if (this.lyricScrollbar.updateScrolling(event)) return true;
        return super.mouseClicked(event, doubled);
    }

    @Override
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        if (this.lyricScrollbar.mouseDragged(event, dragX, dragY)) return true;
        return super.mouseDragged(event, dragX, dragY);
    }

    @Override
    public boolean mouseReleased(MouseButtonEvent event) {
        if (event.button() == 0) this.lyricScrollbar.onRelease(event);
    ...
    private void renderLyrics(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(this.font, Component.translatable("concerto.screen.lyrics_preview"), contentX,
        ...
        if (this.wrappedLyrics.isEmpty()) {
            graphics.centeredText(this.font, Component.translatable("concerto.no_subtitle"),
        ...
        for (int i = 0; i < this.wrappedLyrics.size(); i++) {
            ...
            if (y > contentY - 10 && y < contentY + contentHeight) {
                ...
                graphics.text(this.font, line.text(), contentX, y, line.color());
            }
        }
        graphics.disableScissor();
        this.lyricScrollbar.extractRenderState(graphics, 0, 0, 0);
    }
