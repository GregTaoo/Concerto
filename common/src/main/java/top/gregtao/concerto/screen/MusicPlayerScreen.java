...
public class MusicPlayerScreen extends ConcertoScreen {
        ...
        Button playlistButton = Button.builder(
                ...
                button -> {
                    if (this.minecraft != null) {
                        this.minecraft.gui.setScreen(new MainPlaylistScreen(this));
                    }
                }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if (!hasTrack) {
            ...
            context.centeredText(this.font, text, this.width / 2, this.height / 2, 0xAAAAAAFF);
            this.volumeControl.extractRenderState(context, mouseX, mouseY, delta);
            return;
        }
    ...
    public boolean mouseClicked(@NotNull MouseButtonEvent event, boolean doubled) {
        ...
        if (event.button() == 0 && this.isOverProgressBar(event.x(), event.y()) && this.seekProgress(event.x(), false)) {
    ...
    public boolean mouseDragged(@NotNull MouseButtonEvent event, double dragX, double dragY) {
        if (this.seekingProgress && event.button() == 0) {
            return this.seekProgress(event.x(), false);
        }
    ...
    public boolean mouseReleased(@NotNull MouseButtonEvent event) {
        if (this.seekingProgress && event.button() == 0) {
            ...
            return this.seekProgress(event.x(), true);
        }
    ...
    private void renderTopProgressBar(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        long durationMs = MusicPlayer.INSTANCE.getEffectiveDurationMillis();
        ...
        context.text(this.font, timeText, 6, 8, 0xFFAAAAAA, false);
        ...
        if (hovered) {
            ...
            context.text(this.font, target, textX, barHeight + 5, 0xFF55FFFF, false);
        }
    }

    private void renderStatusHint(GuiGraphicsExtractor context) {
        Component hint = null;
        ...
        if (hint != null) {
            context.centeredText(this.font, hint, this.width / 2, 32, 0xFFAAAAAA);
        }
    }
