...
public class MusicRoomsScreen extends ConcertoScreen {
    ...
    private void renderDiscovery(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        graphics.text(this.font, Component.translatable("concerto.room.list.title"), this.standardContentX(), 22,
                SECTION_LABEL_COLOR, false);
        this.roomList.extractRenderState(graphics, mouseX, mouseY, delta);
    ...
    private void renderRoom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        MusicRoom.MusicRoomState room = MusicRoom.clientGetRoomState();
        if (this.builtState == MusicRoom.ClientState.MUSIC_AGENT) {
            graphics.centeredText(this.font, Component.translatable("concerto.screen.in_music_agent"), this.width / 2, 17,
                    0xffffffff);
            graphics.text(this.font, Component.translatable("concerto.room.members.title"), this.standardContentX(), 28,
                    SECTION_LABEL_COLOR, false);
        } else {
            ...
            graphics.centeredText(this.font, name, this.width / 2, 17, 0xffffffff);
            String uuid = MusicRoom.CLIENT_ROOM.uuid.toString();
            boolean hovering = this.isHoveringUuid(mouseX, mouseY, uuid);
            graphics.centeredText(this.font, Component.literal(uuid).withStyle(style -> style
                    .withColor(ChatFormatting.DARK_GRAY).withUnderlined(hovering)), this.width / 2, 28, 0xff888888);
            graphics.text(this.font, Component.translatable("concerto.room.members.title"), this.standardContentX(), 39,
                    SECTION_LABEL_COLOR, false);
        }
        if (room != null && room.members.hashCode() != this.lastMembersHash) this.resetMembers(room);
        this.memberList.extractRenderState(graphics, mouseX, mouseY, delta);
    ...
    private void drawListMessage(GuiGraphicsExtractor graphics, Component text) {
        graphics.centeredText(this.font, text, this.width / 2,
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        if (event.button() == 0 && this.builtState == MusicRoom.ClientState.MUSIC_ROOM && MusicRoom.CLIENT_ROOM != null) {
            String uuid = MusicRoom.CLIENT_ROOM.uuid.toString();
            if (this.isHoveringUuid(event.x(), event.y(), uuid)) {
