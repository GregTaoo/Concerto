...
public class MainPlaylistWidget extends MetadataListWidget<MainPlaylistWidget.Entry> {
    ...
    @Override
    protected void renderEntry(ConcertoListWidget<Entry>.Entry entry, GuiGraphicsExtractor graphics, int y, int x,
        ...
        graphics.text(this.minecraft.font, title, x, y + 3, 0xffffffff, false);
        if (!author.isEmpty()) {
            graphics.text(this.minecraft.font, Component.literal(author).withStyle(ChatFormatting.BOLD, ChatFormatting.GRAY),
    ...
    private void renderDragFeedback(ConcertoListWidget<Entry>.Entry entry, GuiGraphicsExtractor graphics, int y, int x, int entryWidth) {
    ...
    private void renderActionBackground(GuiGraphicsExtractor graphics, int x, int y, boolean hovered) {
        graphics.fill(x, y, x + ACTION_BUTTON_WIDTH, y + ACTION_BUTTON_WIDTH,
                hovered ? 0xb0303030 : 0x80606060);
    }

    private void renderPlayButton(GuiGraphicsExtractor graphics, int x, int y, boolean hovered) {
        this.renderActionBackground(graphics, x, y, hovered);
        graphics.centeredText(this.minecraft.font, Component.literal("▶"), x + ACTION_BUTTON_WIDTH / 2, y + 3,
                0xffffffff);
    }

    private void renderEditButton(GuiGraphicsExtractor graphics, int x, int y, boolean hovered) {
        this.renderActionBackground(graphics, x, y, hovered);
        graphics.centeredText(this.minecraft.font, Component.literal("✎"), x + ACTION_BUTTON_WIDTH / 2, y + 3,
                0xffffffff);
    }

    private void renderTrashButton(GuiGraphicsExtractor graphics, int x, int y, boolean hovered) {
        this.renderActionBackground(graphics, x, y, hovered);
        graphics.centeredText(this.minecraft.font, Component.literal("🗑"), x + ACTION_BUTTON_WIDTH / 2, y + 3,
                0xffffffff);
    }
