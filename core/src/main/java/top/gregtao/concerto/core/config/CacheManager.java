...
public class CacheManager {
    ...
    public static void cleanAllCache() {
        ...
        if (!Files.exists(cacheRoot) || !Files.isDirectory(cacheRoot)) {
            Concerto.getLogger().error("Cache folder does not exist or is not a directory: {}", cacheRoot);
            return;
        }
        ...
        try (Stream<Path> walk = Files.walk(cacheRoot, 1)) {
            ...
            Concerto.getLogger().error("Failed to scan cache folder: {}", cacheRoot, e);
        }
    }
        ...
        try (Stream<Path> walk = Files.walk(path)) {
            walk.sorted(Comparator.reverseOrder()).forEach(p -> {
                try {
                    ...
                    Concerto.getLogger().error("Failed to delete: {}", p, e);
                }
            });
        } catch (IOException e) {
            Concerto.getLogger().error("Failed to traverse path: {}", path, e);
        }
    }
    ...
    public void removeEarliest() {
        ...
        List<Pair<Pair<File, Long>, Long>> list = Arrays.stream(files).filter(File::isFile).map(file -> {
            ...
            try {
                ...
                Concerto.getLogger().warn("Error occurs while trying removing a file", e);
        ...
        while (finalSize > this.maxSize && pos < list.size()) {
            ...
            if (pair.getFirst().delete()) {
                ...
                Concerto.getLogger().warn("Cannot remove file {}", pair.getFirst().getAbsolutePath());
            }
            ++pos;
        }
    }
