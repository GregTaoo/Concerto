...
public class KuGouMusicApiClient extends HttpApiClient {
    ...
    public static String GUID = HashUtil.md5(getGuid());

    public static String MID = calculateMid(GUID);
    ...
    public Optional<JsonObject> getMusicDetail(String albumAudioId, String field) {
        if (isStringEmpty(albumAudioId)) return Optional.empty();
    ...
    public Optional<JsonObject> getPlayListAllTrack(String id) {
        if (isStringEmpty(id)) return Optional.empty();
                ...
                .map(arr -> StreamSupport.stream(arr.spliterator(), false)
                        .map(element -> {
                            ...
                            MusicMetaData musicMetaData = songOpt.map(KuGouMusic::parseMetaData)
                                    .orElse(new UnknownMusicMeta(Sources.KUGOU_MUSIC.getName()));
    ...
    public Optional<Pair<Long, String>> cellphoneLogin(String mobile, String code) {
        if (isStringEmpty(mobile) || isStringEmpty(code) || mobile.length() < 11) return Optional.empty();
    ...
    public Optional<Pair<Long, String>> login(String userName, String password) {
        if (isStringEmpty(userName) || isStringEmpty(password)) return Optional.empty();
