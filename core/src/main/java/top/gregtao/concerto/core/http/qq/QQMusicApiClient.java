...
public class QQMusicApiClient extends HttpApiClient {
    ...
    public String combineWeChatQRLink(String key) {
        try {
            ...
            Concerto.getLogger().warn("Error while combining WeChat QR link", e);
            return "error";
        }
    }
    ...
    public static String randomNumber(int length, boolean with0) {
        return with0 ? RandomUtil.randomString(length, "0123456789") : RandomUtil.randomString(length, "123456789");
    }
    ...
    public JsonObject search(String keyword, SearchType type, int page) {
        try {
            ...
            Concerto.getLogger().warn("Error searching for music {}", keyword, e);
            return null;
        }
    }
