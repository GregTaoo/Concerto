...
public class EnumArgumentType<T extends Enum<T> & StringIdentifiable> implements ArgumentType<T> {
    private static final DynamicCommandExceptionType INVALID_ENUM_EXCEPTION = new DynamicCommandExceptionType(
            (value) -> new TranslatableText("argument.enum.invalid", value));
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(Arrays.stream(this.valuesSupplier.get())
                .map(StringIdentifiable::asString).map(this::transformValueName).collect(Collectors.toList()), builder);
    }

    public Collection<String> getExamples() {
        return Arrays.stream(this.valuesSupplier.get()).map(StringIdentifiable::asString)
