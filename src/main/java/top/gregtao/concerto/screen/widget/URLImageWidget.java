...
public class URLImageWidget implements Drawable, Widget, AutoCloseable {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.border) DrawableHelper.drawBorder(matrices, this.x, this.y, this.width, this.height, 0xffffffff);
        TextRenderer textRenderer = MinecraftClient.getInstance().textRenderer;
        if (this.url == null || this.texture == null) {
            DrawableHelper.drawCenteredTextWithShadow(
                    matrices, textRenderer, Text.translatable("concerto.screen.url_image.empty"),
            ...
            if (image != null && this.state == State.READY) {
                ...
                DrawableHelper.drawCenteredTextWithShadow(
                        matrices, textRenderer, Text.translatable("concerto.screen.loading"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.fontHeight) / 2, 0xffffffff
                );
            } else {
                DrawableHelper.drawCenteredTextWithShadow(
                        matrices, textRenderer, Text.translatable("concerto.fail"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.fontHeight) / 2, 0xffffffff
                );
            }
        }
